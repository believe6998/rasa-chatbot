...
def getListTable():
    db = MySQLdb.connect("localhost", "root", "nam321321", "chatbot_rasa")
    cursor = db.cursor()
...
def Menu():
    ...
    f1 = Food("Lẩu dầu cay không vụn", 200, "hotpot", 0,
              "Lẩu dầu cay là hương vị truyền thống ở MTee Ay, tuy nhiên, nước lẩu này khá cay, nhiều dầu và gia vị, các bạn không quen ăn đồ Trung Quốc và ăn được ít cay thì nên cân nhắc ")
    f2 = Food("Lẩu cay mala", 250, "hotpot", 0,
              "Dạ trong tiếng Trung, 'ma' là tê , 'la' là cay, Mala tức là cay đến tê người.Lẩu cay thơm từ hạt ngò, thảo quả, kỳ tử,… ")
    f3 = Food("Lẩu thảo dược", 230, "hotpot", 0,
              "Lẩu thảo dược được nầu từ nguyên liệu là các thảo dược Trung Hoa rất tốt cho sức khỏe ạ")
...
class OrderTable(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if day is None:
            dispatcher.utter_message("Bạn muốn đặt bàn vào thứ mấy nhỉ ?")
            return []

        if moment is None:
            dispatcher.utter_message(
                "Bên mình chỉ cho đặt trước 2 tuần nên là {} tuần này hay tuần sau bạn ơi ".format(day))
            return []

        if hour is None:
            dispatcher.utter_message("Bạn đi lúc mấy giờ ạ ? ")
            return []
...
class ConfirmOrderTable(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        dispatcher.utter_message("Order bàn thành công rồi nhé. Mình sẽ liên hệ lại sớm để xác nhận nhé")
        dispatcher.utter_message("Anh/Chị : " + cusName + "\nSố Điện Thoại: " + cusPhone + "\nSố Người: " + quanity[
            0] + "\nMã bàn : " + table_for_order + "\nGhi chú:{} - {} - {} ".format(hour, day, moment))

        return [SlotSet("order_table", None), SlotSet("quanity", None), SlotSet("sOrderTable", None),
                SlotSet("cusName", None), SlotSet("cusPhone", None), SlotSet("hour", None), SlotSet("day", None),
                SlotSet("moment", None)]
...
class SugestOrderFood(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        context_1 = tracker.get_slot('sOrderFood')
        context_2 = tracker.get_slot('sOrderTable')
        print("action_suggest_order_food")
